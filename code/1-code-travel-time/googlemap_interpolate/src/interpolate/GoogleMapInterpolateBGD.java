 ...
 * 
 * Description: 
 * 	Create interpolated tower pair data of google map travel time in traffic, travel time and travel distance in Bangladesh
 */
...
public class GoogleMapInterpolateBGD {

	public static void main(String[] args) throws IOException  {
		// Input files
			String path = "C:/Users/Yuhei/Dropbox (MIT)/research/BGD_SLK_cellphone/data/coded_bgd/googlemap/"; 
			String file1 = path + "all tower pair in Dhaka before interpolation.csv";
			String file2 = path + "random tower pair - google prediction before interpolation.csv";
			
		// Output files
			String outfile = path + "all tower pair in Dhaka after interpolation.csv";
			String outfile_aux = path + "all tower pair in Dhaka after interpolation auxiliary.csv";
		...
		// INPUT DATA
			...
			while ((line = br.readLine()) != null) {
				...
				// process the line : dest_long dest_lat dest_tower orig_long orig_lat orig_tower dist duration_bg distance_bg h_d h_o
				...
				//add assign index to orig and dest if haven't done so already
					...
					if(!tower2n.containsKey(d_temp)){
						...
						lon[itower]=Double.valueOf(parts[0]);
						lat[itower]=Double.valueOf(parts[1]);
						h[itower]=Double.valueOf(parts[9]);
						n2tower[itower] = d_temp;
						tower2n.put(d_temp, itower++);
					}					
					if(!tower2n.containsKey(o_temp)){
						...
						lon[itower]=Double.valueOf(parts[3]);
						lat[itower]=Double.valueOf(parts[4]);
						h[itower]=Double.valueOf(parts[10]);
						n2tower[itower] = o_temp;
						tower2n.put(o_temp, itower++);
					}

				// save duration_in_traffic_orig
					duration_orig[tower2n.get(o_temp)][tower2n.get(d_temp)] = Double.valueOf(parts[7]);
					distance_orig[tower2n.get(o_temp)][tower2n.get(d_temp)] = Double.valueOf(parts[8]);
		...
		bfw.write("orig,dest,sldist,duration_orig,duration_intp,distance_orig,distance_intp");
		bfw.newLine();
		
		for(int i=0;i<n_towers;i++)
			for(int j=0;j<n_towers;j++){
				if(sldist[i][j]>0){
					bfw.write(n2tower[i] + "," + n2tower[j] + "," + sldist[i][j]+ 
