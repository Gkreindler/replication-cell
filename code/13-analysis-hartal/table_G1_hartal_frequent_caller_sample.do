* this do file: Robustness for hartal analysis (table G1)
...
*** Normalize commuting outcome var relative to non-holiday non-hartal workdays
	...
	*** Only FREQUENT commuters, above 50% days
	keep if above50_new == 1
...
*** Write to file
	esttab c1 c2 c3 c3b c4 using "tables/table_5/main_table_heterogeneity_G1.tex", ///
		replace b(%12.3f) se(%12.3f) drop(_cons ratio_ols_high log_dur_share_low log_dur_share_high destfe_share_high destfe_share_low) ///
			...
			coeflabels(hartal "Hartal" ///
						hartal_share_low    "\quad ($\beta^L$) Hartal $\times$ \% Low Skill"  ///
						hartal_share_high   "\quad ($\beta^H$) Hartal $\times$ \% High Skill"  ///
					   hartal_destfe     	"\quad Hartal $\times$ Dest. FE (z) " ///
					   h_destfe_share_low   "\quad ($\beta_W^L$) Hartal $\times$ \% Low Skill $\times$ Dest. FE Low Skill (z)" ///
					   h_destfe_share_high  "\quad ($\beta_W^H$) Hartal $\times$ \% High Skill $\times$ Dest. FE High Skill (z)" ///
					   hartal_log_dur 		"\quad Hartal $\times$ Log Duration (z)"  ///
					   hartal_log_d2cbd     "\quad Log Workplace Distance to CBD (z) " ///
					   hartal_log_dur_share_low  "\quad ($\beta_D^L$) Hartal $\times$ \% Low Skill $\times$ Log Duration (z) "  ///
					   hartal_log_dur_share_high "\quad ($\beta_D^H$) Hartal $\times$ \% High Skill $\times$ Log Duration (z) "  ///
					   ) ///
			starlevels("$^{*}$" .10 "$^{**}$" .05 "$^{***}$" .01)  ///
